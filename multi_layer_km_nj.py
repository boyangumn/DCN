...
def batch_km(data, center):
    ...
    return idx, center_new
        
def test_SdC_NJ(lbd = 0, finetune_lr= .005, mu = 0.9, pretraining_epochs=50,
...
#    count = numpy.zeros(nClass, dtype = numpy.int)
...
#            lr_shared.set_value( numpy.float32(finetune_lr/numpy.sqrt(epoch)) )
            ...
            temp_idx, centers = batch_km(hidden_val, centers)
...
#        print 'Learning rate: %.6f' %numpy.mean(f)
    ...
    hidden_size = hidden_array.shape        
    hidden_array = numpy.reshape(hidden_array, (hidden_size[0] * hidden_size[1], hidden_size[2] ))
    # Run a Kmeans on the hidden_array    
    km.fit_predict(hidden_array)

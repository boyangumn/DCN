...
from multi_layer_km import load_all_data
...
#    def finetune_cost_updates(self, center, learning_rate):
...
#        network_output = self.dA_layers[-1].get_hidden_values(self.dA_layers[-1].x)
...
def batch_mmc(prototypes, data, eta, lr):
    ...
    new_assignment = numpy.zeros_like(assignment) 
    new_prototypes = numpy.zeros_like(prototypes)
    ...
    for k in range(K):
        ...
        for n in range(N):
            if new_assignment[n, 0] == k:
                grad += eta*prototypes[k] - data[n]
            elif new_assignmen[n,1] == k:
                grad += eta*prototypes[k] + data[n]
            else:
    ...
    return new_prototypes, new_assignment      
    
           
def test_SdC(lbd = .01, finetune_lr= .005, mu = 0.9, pretraining_epochs=50,
             pretrain_lr=.001, training_epochs=150, init = 'rbm_init_30',
...
#    ypred = km.predict(hidden_array)   
    ...
    ari_dc = metrics.adjusted_rand_score(label_true, km_idx)
